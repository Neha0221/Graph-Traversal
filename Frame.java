...
private String algo[] = { "A*","DFS" };
	...
	public Frame() {
		ch = new ControlHandler(this, algo);
		size = 25;
		mode = "Map Creation";
		showSteps = true;
		btnHover = false;
		...
		window.setTitle("Graph Traversal Visualization");
	...
	public void paintComponent(Graphics g) {
		...
		g.fillRect(10, height - 96, 400, 90);
	...
	public void MapCalculations(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			// If 's' is presesed create start node
			if (currentKey == 's') {
				int xBorder = e.getX() - (e.getX() % size);
				int yBorder = e.getY() - (e.getY() % size);

				if (startNode == null) {
					startNode = new Node(xBorder, yBorder);
				} else {
					startNode.setXY(xBorder, yBorder);
				}
				repaint();
			}
			// If 'e' is pressed create end node
				...
				if (endNode == null) {
					endNode = new Node(xBorder, yBorder);
				} else {
					endNode.setXY(xBorder, yBorder);
				}
				repaint();
			}
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (x >= 10 && x <= 410 && y >= (height - 96) && y <= (height - 6)) {
			btnHover = true;
		} else {
			btnHover = false;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (pathfinding.isRunning() && showSteps) {
			pathfinding.findPath(pathfinding.getCurNode());
			mode = "Running";
		}
		...
		if (e.getActionCommand() != null) {
			...
			if (e.getActionCommand().equals("run") && !pathfinding.isRunning()) {
				...
				ch.getB("run").setText("stop");
				timer.start();
			} else if (e.getActionCommand().equals("comboBoxChanged") && !pathfinding.isRunning()) {
				String selectedAlgo = algo[ch.getComboBox().getSelectedIndex()];
				Strategy newAlgo = algoHashMap.get(selectedAlgo);
				pathfinding.copyNode(newAlgo);
				pathfinding = newAlgo;
			}
		}
		repaint();
